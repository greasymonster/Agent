...
def get_new_search_queries(user_query, previous_search_queries, all_contexts):
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {"role": "system", "content": "You are an expert in extracting and summarizing relevant information."},
            {"role": "user", "content": f"user query {user_query}\nPrecious Search Queries:{previous_search_queries}\n\nExtracted Relevant Contexts:\n{context_combined}\n\n{prompt}"}
        ]
    )
...
def web_search(query: str, top_k: int=2, categories: str="general") -> list[str]:
...
def fetch_webpage_text(url):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            text = response.text
            logger.info(f"Jina fetch error for {url}: {response.status_code} - {text}")
            return ""
    except Exception as e:
...
def process_link(link, query, search_query):
    ...
    usefulness = if_useful(query=query, page_text=page_text)
    logger.info(f"Page usefulness for {link}: {usefulness}")
    if usefulness == "Yes":
        context = extract_relevant_context(query=query, search_query=search_query, page_text=page_text)
        if context:
            logger.info(f"Extract context from {link} (first 200 chars): {context[:200]}")
            return context
...
def search(query: str) -> str:
    ...
    new_search_queries = eval(generate_query(query=query))
    all_search_queries.extend(new_search_queries)

    if query not in all_search_queries:
        all_search_queries.append(query)
    
    while iteration < iteration_limits:
        logger.info(f"\n====Iteration{iteration + 1}====")
        iteration_contexts = []
        search_results = [web_search(query=query, top_k=2, categories='general') for query in new_search_queries]
        ...
        for idx, links in enumerate(search_results):
            ...
            for link in links:
                if link not in unique_links:
                    unique_links[idx] = search_query
        
        logger.info(f"Aggregated {len(unique_links)} unique links from this iteration.")

        link_results = [
            process_link(link=link, query=query, search_query=unique_links[link]) for link in unique_links
        ]
        ...
        new_search_queries = get_new_search_queries(user_query=query, previous_search_queries=all_search_queries, all_contexts=aggregated_contexts)
        if new_search_queries == "":
            logger.info(f"LLM indicated that no further research is needed.")
            break
...
def get_images(query: str) -> str:
    ...
    img_srcs = web_search(query=query, top_k=2, categories="general")

    results = {}

    for img_src in img_srcs:
        logger.info(f"Fetching image description for: {img_src}")
        description = get_images_description(image_url=img_src)
        logger.info(f"Image description for {img_src}: {description}")
        results[img_src] = description
