...
def cron_matches(cron_expr: str, dt: datetime) -> bool:
    ...
    if dow_unconstrained and dom_unconstrained:
        return True
...
def _validata_corn_field(field: str, lo: int, hi: int) -> str | None:
    if field == "*":
        return None
    if field.startswith("*/"):
        step_str = field[2:]
    ...
    if "," in field:
        for part in field.split(","):
            err = _validata_corn_field(part.strip(), lo, hi)
            if err:
                return err
    if "-" in field:
        parts = field.split("-", 1)
        if not parts[0].isdigit() or not part[1].isdigit():
...
def validate_cron(cron_expr: str) -> str | None:
    ...
    for i, (field, (lo, hi), name) in enumerate(zip(fields, bounds, names)):
        err = _validata_corn_field(field=field, lo=lo, hi=hi)
        if err:
            return f"{name}: {err}"
