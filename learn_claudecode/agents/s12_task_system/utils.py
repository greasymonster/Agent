...
def load_task(task_id: str) -> Task:
    return Task(**json.loads(_task_path(task_id=task_id).read_text()))
...
def claim_task(task_id: str, owner: str = "agent")-> str:
    ...
    if not can_start(task_id):
        deps = [
            d for d in task.blockedBy if not _task_path(d).exists() or load_task(d).status != "completed"
        ]
        return f"Blocked by {deps}"
    task.owner = owner
    task.status = "in_peogress"
    save_task(task)
...
def complete_task(task_id: str) -> str:
    ...
    unblocked = [t.subject for t in list_tasks()
                 if t.status == "pending" and t.blockedBsy and can_start(t.id)]
