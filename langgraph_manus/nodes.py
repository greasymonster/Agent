...
def create_planner_node(state: State):
    ...
    messages = [SystemMessage(content=PLAN_CREATE_PROMPT), HumanMessage(content=PLAN_CREATE_PROMPT.format(user_message=state['user_message']))]
...
def execute_node(state: State):
    ...
    logger.info(f"当前执行的STEP:{current_step}")
    ...
    while True:
        ...
        if response["tool_calls"]:
            ...
            logger.info(f"tool_name:{tool_name},tool_args:{tool_args}\ntool_result:{tool_result}")
            messages += [AIMessage(content=extract_answer(response['content']))]
            messages += [ToolMessage(content=f"tool_result:{tool_result}")]
